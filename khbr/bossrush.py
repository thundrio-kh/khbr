...
def main(cli_args: list=[]):
    ...
    options.add_argument("-starting_room", choices=["random"] + bosses, default=last_settings.get("starting_room"))
    ...
    options.add_argument("-randomize_room_order", action="store_true", default=last_settings.get("randomize_room_order"))
    ...
    settings_to_write = {
        ...
        "level": args.level,
        "randomize_room_order": args.randomize_room_order,
    ...
    for i in range(num_bosses):
        boss_order.append(bosses[(index + i) % len(bosses)] if not randomize_room_order else random.choice(bosses))
    ...
    for b in range(len(boss_order)): # changes to boss_order
        ...
        assetgenerator.generateEvt(world, room, program, asset["source"], options={"jump_to":{"world": newworld, "room": newroom, "program": newprogram}, "open_menu":open_menu_before_each_fight, "remove_event":True})
        world = newworld
        room = newroom
        program = newprogram
    
    assetgenerator.generateEvt(world, room, program, asset["source"], options={"jump_to":{"world": "ES", "room": "00", "program": 69}})
